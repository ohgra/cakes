...
# =====================================================
def augment_embeddings_with_parents(
        embeddings,
        W_dag,
        retrieval_vector,   # unused
        top_k=5):

    d, dim = embeddings.shape

    out = torch.zeros((d, dim),
    ...
    for j in range(d):
        ...
        idxs = torch.argsort(torch.abs(w),
                             descending=True)[:top_k]

        idxs = idxs[w[idxs].abs() > 1e-6]

        parent_vec = torch.zeros(dim,
                                 device=embeddings.device)

        if len(idxs) > 0:
            probs = w[idxs] / (torch.sum(torch.abs(w[idxs])) + 1e-8)
            parent_vec = torch.sum(
                probs.unsqueeze(1) * embeddings[idxs],
                dim=0
            )
            parent_vec = l2_normalize(parent_vec.unsqueeze(0))[0]

        # combined = embeddings[j]
        combined = embeddings[j] + parent_vec
        out[j] = l2_normalize(combined.unsqueeze(0))[0]
...
if __name__ == "__main__":
    process_npz_folder("clip_dgm4_gemma_test", "clip_dgm4_gemma_test_aug")
    print("Done")